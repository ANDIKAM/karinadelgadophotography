...
+function ($) { "use strict";
    ...
    var OctoberLeaves = function(element, options) {
        ...
        this.timer = null;
        this.winWidth =$(this.options.container).width();
        this.winHeight = $(this.options.container).height();
    ...
    OctoberLeaves.prototype.start = function() {
        ...
        for (var i = 0 ;i < this.options.numberOfLeaves;i++){
            ...
            $(this.options.container).append(img);
        }
    ...
    OctoberLeaves.prototype.leafCycle = function() {
        for (var i = 0; i < this.options.numberOfLeaves; i++) {
            ...
            if (Ypos[i] < 0){
                opacityLeaf[i] = 1;
                $(this.options.container+' #octoberLeaf'+i).css({opacity:opacityLeaf[i]});
            }
            ...
            if (this.options.rotationTrue == 1){
                $(this.options.container+' #octoberLeaf'+i).css({top: Ypos[i], left: Xpos[i]});
                $(this.options.container+' #octoberLeaf'+i).rotate3Di(rotationAll[i], 0);
            }
